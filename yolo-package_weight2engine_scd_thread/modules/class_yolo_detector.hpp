...
private:
	...
	void parse_config()
	{
		...
		if (_config.net_type == tensor_rt::YOLOV5s_cls || _config.net_type == tensor_rt::YOLOV5s_seg) {
			auto npos = _yolo_info.onnxPath.find(".onnx");
			assert(npos != std::string::npos
				&& "onnx file file not recognised. File needs to be of '.onnx' format");
			_yolo_info.data_path = _yolo_info.onnxPath.substr(0, npos);
		}
	...
	void build_net()
	{
		...
		else if (_config.net_type == tensor_rt::YOLOV5s_seg) {
			std::cout << "YOLOV5s_seg start build net" << std::endl;
			_p_net = std::unique_ptr<Yolo>{ new YoloV5s_seg(_yolo_info,_infer_param) };
		}
		else
		{
...
private:
	...
	std::vector<std::string> _vec_net_type{ "yolov3", "yolov4", "yolov4-tiny", "yolov5", "yolov5s_cls", "yolov5s_seg"};
