...
Yolo::Yolo(const NetworkInfo& networkInfo, const InferParams& inferParams)
    : m_NetworkType(networkInfo.networkType),
	  ...
	  m_onnxPath(networkInfo.onnxPath),
      ...
      m_BatchSize(inferParams.batchSize)
{
	...
	if (m_NetworkType == "yolov5s_cls" || m_NetworkType == "yolov5s_seg") {
		m_EnginePath = networkInfo.data_path + "-" + m_Precision + "-batch" + std::to_string(m_BatchSize) + ".engine";

		if (m_Precision == "kHALF") {
			createYOLOv5s_clsEngine(m_onnxPath.c_str(), nvinfer1::DataType::kHALF);
		}
		if (m_Precision == "kFLOAT") {
			createYOLOv5s_clsEngine(m_onnxPath.c_str(), nvinfer1::DataType::kFLOAT);
		}
...
void Yolo::createYOLOv5s_clsEngine(const char* onnx_filename, const nvinfer1::DataType dataType)
{
	...
	std::ofstream p(m_EnginePath, std::ios::binary);
	if (!p)
	{
...
//void Yolo::thread_infer(const unsigned char* input, const uint32_t batchSize)
//{
...
//	cudaMallocHost((void**)&output, NULL);
//	//锁定GPU
//	cudaMalloc(&buffer[inputIndex], size);
//	cudaMalloc(&buffer[outputIndex], size);
//	//异步推理
//	for (int i = 0; i < 3; i++) {
//		cudaMemcpyAsync(buffer[inputIndex], hostDataBuffer, size, cudaMemcpyHostToDevice, stream[i]);
//		contextt->enqueueV2(buffer, stream[i], nullptr);
//		cudaMemcpyAsync(output, buffer[outputIndex], size, cudaMemcpyDeviceToHost, stream[i]);
//	}
//	//同步流
//	for (int i = 0; i < 3; i++) {
