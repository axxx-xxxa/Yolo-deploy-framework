...
public:
	...
	void detect_cls(const std::vector<cv::Mat>& vec_image,
		std::vector<tensor_rt::BatchResult_yolo>& vec_batch_result);
	
	void detect_seg(const std::vector<cv::Mat>& vec_image,
		std::vector<tensor_rt::BatchResult_yolo>& vec_batch_result);
	...
	void get_m_n(std::string& model_name);

	void release();

	std::string getModelType() {
		return _yolo_info.networkType;
	}
private:
...
private:
	...
	std::vector<std::string> _vec_net_type{ "yolov3", "yolov4", "yolov4-tiny", "yolov5", "yolov5s_cls" };
...
public:
	...
	bool thread_init(const std::string manager_yaml_path, ConcurrenceQueue<std::string>& Result_queue);

	bool thread_infer(int thread_id, std::vector<cv::Mat>& imgs);
...
private:
	static void inthread_infer(std::vector<std::string> yaml_path, ConcurrenceQueue<std::string>& Cqueue, ConcurrenceQueue<std::vector<cv::Mat>>& produce_queue, int thread_id);

	std::vector<std::thread> threads;

	std::map<int, std::vector<int>> LTmap;

	ConcurrenceQueue<std::vector<cv::Mat>> producer_queues[100];
