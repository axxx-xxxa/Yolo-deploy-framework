...
// ����Yolo(const NetworkInfo& networkInfo, const InferParams& inferParams)���ù��캯��
// �������Ǹ���Ա������ֵ
// ��ֵ֮��ִ�й��캯���ڵ�����
Yolo::Yolo(const NetworkInfo& networkInfo, const InferParams& inferParams)
    : m_NetworkType(networkInfo.networkType),
	  ...
	  m_EngineFilePath(networkInfo.enginePath),
      ...
      m_BatchSize(inferParams.batchSize)
{
	...
	// ��������cfgģ����Ϣ��������
	if (m_NetworkType == "yolov5s_cls") 
	{
		m_InputH = networkInfo.input_h;
		m_InputW = networkInfo.input_w;
		_n_classes = networkInfo.classes;
		m_BatchSize = inferParams.batchSize;

		m_EnginePath = m_EngineFilePath;
		
		m_Engine = loadTRTEngine(m_EnginePath,/* m_PluginFactory,*/ m_Logger, m_NetworkType);
...
std::vector<double> Yolo::softmax(std::vector<double> input)
...
void Yolo::doInference_v5_seg(float* input, float* output, int batchSize)
{
	int OUTPUT_SIZE = _n_classes;//output0

	const char* INPUT_BLOB_NAME = "images";
	const char* OUTPUT_BLOB_NAME = "output0";//class
	...
	const int outputIndex = m_Engine->getBindingIndex(OUTPUT_BLOB_NAME);

	// Create GPU buffers on device
	NV_CUDA_CHECK(cudaMalloc(&buffers[inputIndex], batchSize * 3 * m_InputH * m_InputW * sizeof(float)));//
	NV_CUDA_CHECK(cudaMalloc(&buffers[outputIndex], batchSize * OUTPUT_SIZE * sizeof(float)));
	...
	NV_CUDA_CHECK(cudaMemcpyAsync(buffers[inputIndex], input, batchSize * 3 * m_InputH * m_InputW * sizeof(float), cudaMemcpyHostToDevice, stream));
	...
	float* bindings[] = { buffers[inputIndex], buffers[outputIndex] };

	m_Context->enqueueV2((void**)bindings, stream, nullptr);

	NV_CUDA_CHECK(cudaMemcpyAsync(output, buffers[outputIndex], batchSize * OUTPUT_SIZE * sizeof(float), cudaMemcpyDeviceToHost, stream));
	cudaStreamSynchronize(stream);

	// Release stream and buffers
	cudaStreamDestroy(stream);
	NV_CUDA_CHECK(cudaFree(buffers[inputIndex]));
	NV_CUDA_CHECK(cudaFree(buffers[outputIndex]));

}

void Yolo::doInference_v5_cls(float* input, float* output, int batchSize)
{
	int OUTPUT_SIZE = _n_classes;//output0
...
//	float output_data_host[18 * num_classes];
//	float* output_data_device = nullptr;
//	NV_CUDA_CHECK(cudaMallocHost(&input_data_host, input_numel * sizeof(float)));
//	NV_CUDA_CHECK(cudaMalloc(&input_data_device, input_numel * sizeof(float)));
//
//
//	for (int i = 0; i < batchSize * 3 * input_height * input_width; i++) {
//		input_data_host[i] = 2.0;
//	}
//
//	NV_CUDA_CHECK(cudaMemcpyAsync(input_data_device, input_data_host, input_numel * sizeof(float), cudaMemcpyHostToDevice, stream));
//
//
//
//	NV_CUDA_CHECK(cudaMalloc(&output_data_device, sizeof(output_data_host)));
//	auto input_dims = m_Engine->getBindingDimensions(0);
//	input_dims.d[0] = batchSize;
//	m_Context->setBindingDimensions(0, input_dims);
//	float* bindings[] = { input_data_device, output_data_device };
//	bool success = m_Context->enqueueV2((void**)bindings, stream, nullptr);
//	NV_CUDA_CHECK(cudaMemcpyAsync(output_data_host, output_data_device, sizeof(output_data_host), cudaMemcpyDeviceToHost, stream));
...
//	}
//	std::cout << std::endl;
//	NV_CUDA_CHECK(cudaFreeHost(input_data_host));
//	NV_CUDA_CHECK(cudaFree(input_data_device));
//	NV_CUDA_CHECK(cudaFree(output_data_device));
//}
//
...
/* dynamic batch
void Yolo::doInference_v5_cls(const unsigned char* input, float* output, int batchSize)
{
	std::cout << m_Engine->getMaxBatchSize() << "--==--" << std::endl;
	std::cout << batchSize << "--==--" << std::endl;
	assert(batchSize <= m_BatchSize && "Image batch size exceeds TRT engines batch size");

	int OUTPUT_SIZE = batchSize * _n_classes;//output0
	...
	// Create GPU buffers on device
	NV_CUDA_CHECK(cudaMalloc(&buffers[inputIndex], batchSize * 3 * m_InputH * m_InputW * sizeof(unsigned char)));//
	NV_CUDA_CHECK(cudaMalloc(&buffers[outputIndex], batchSize * OUTPUT_SIZE * sizeof(float)));
	// cudaMalloc分配内存 cudaFree释放内存 cudaMemcpy或 cudaMemcpyAsync 在主机和设备之间传输数据
	// cudaMemcpy cudaMemcpyAsync 显式地阻塞传输 显式地非阻塞传输 
	// Create stream
	...
	// DMA input batch data to device, infer on the batch asynchronously, and DMA output back to host
	NV_CUDA_CHECK(cudaMemcpyAsync(buffers[inputIndex], input, batchSize * 3 * m_InputH * m_InputW * sizeof(unsigned char), cudaMemcpyHostToDevice, stream));
	m_Context->enqueue(batchSize, buffers, stream, nullptr);
	NV_CUDA_CHECK(cudaMemcpyAsync(output, buffers[outputIndex], batchSize * OUTPUT_SIZE * sizeof(float), cudaMemcpyDeviceToHost, stream));
	cudaStreamSynchronize(stream);
